...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Setup auth enabled toggle
        toggleAuthEnabled.setOnCheckedChangeListener { _, isChecked ->
            toggleAuthEnabled(isChecked)
        }
    ...
    private fun toggleAuthEnabled(enabled: Boolean) {
        try {
            val configManager = ConfigManager.getInstance(this)
            configManager.setAuthEnabledWithNotification(enabled)
            Log.d("DROIDRUN_MAIN", "Auth enabled toggled to: $enabled")
            Toast.makeText(this, if (enabled) "Token验证已开启" else "Token验证已关闭", Toast.LENGTH_SHORT).show()
        } catch (e: Exception) {
            Log.e("DROIDRUN_MAIN", "Error toggling auth enabled: ${e.message}")
        }
    }

    private fun syncAuthEnabledToggle() {
        val configManager = ConfigManager.getInstance(this)
        toggleAuthEnabled.isChecked = configManager.authEnabled
    }
