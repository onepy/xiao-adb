    ...
    private fun handleClient(clientSocket: Socket) {
        try {
            clientSocket.use { socket ->
                ...
                // Validate Auth Token if authentication is enabled
                // Always skip token validation for /ping endpoint
                if (configManager.authEnabled && path != "/ping" && authToken != configManager.authToken) {
